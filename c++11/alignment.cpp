 ...
 * она должна занимать 8 байт. Однако, это на самом деле это не обязательно и
 * зависит от того, будет ли структура выравнена.
 * 
 * Если данные выравниваются по 4 байтам, то первое поле должно быть дополнено
 * одним байтом, а последнее тремя. В итоге 8 + 4 = 12 байт.
 */
struct data_test_2 {
    char    m_char;         // 1 байт
...
/*
 * Директива #pragma pack заставляет компилятор не выравнивать поля в структурах.
 * Таким образом, следующая структура будет занимать в памяти ровно 8 байт.
 */
#pragma pack(push,1)
struct data_test_4 {
    char    m_char;
