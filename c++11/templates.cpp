...
namespace ex6 {
    ...
    /* Пакет вариативного шаблона может быть распакован рекурсивно. Эта идиома
     * реализуется всегда парой функций: одна из них является стоповой и обычно
     * имеет пустое тело; вторая функция имеет два аргумента и реализует бработку аргументов пакета.
     * Обе функции должны иметь одинаковые имена.
     * 
     * Далее показан пример рекурсивной распаковки.
     */
    void rprint() {
        /*
         * Тело этой функции обычно пустое. Она нужна, чтобы остановить рекурсию.
         */
    }
    template<typename Arg, typename ...Args>
    void rprint(Arg&& arg, Args&&... args)
    {
        // здесь мы реализуем операции над очередным значением пакета
        std::cout << arg << std::endl;
        /*
         * Затем мы должны рекурсивно вызвать эту функцию. При этом новый
         * пакет, который ей будет передан вторым аргументом, как бы сдвинется на позицию вправо.
         * Рекурсия будет завершена, когда пакет будет развернут пустым.
         */
        rprint(args...);
    }
}
